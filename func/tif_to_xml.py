...
# --- 核心函数：提取变化区域的精确外轮廓信息 ---
def get_change_area_contours(image_array, geo_transform, proj_wkt):
    """
    识别图像数组中所有非背景区域，提取它们的精确多边形外轮廓点集，
    并将其转换为地理（WGS84）坐标。

    参数:
        image_array (np.ndarray): 变化检测结果图像数组 (H, W)，像素值代表变化类型。
                                  像素值：1=不变，2=新建，3=扩建，4=损毁，5=拆除
            ...
            for p_x, p_y in pixel_contour_points:
                ...
                if coord_transform:
                    try:
                        ...
                        lat, lon, _ = coord_transform.TransformPoint(orig_geo_x, orig_geo_y)
...
def convert_tif_to_xml(tif_path: str, output_xml_path: str):
    """
    将 GeoTIFF 变化检测结果文件转换为指定的 XML 格式。
    它识别所有非背景区域，并提供其精确外轮廓点集（像素和地理）。

    参数:
        tif_path (str): GeoTIFF 变化检测结果文件的路径。
                        预期像素值：1=不变，2=新建，3=扩建，4=损毁，5=拆除
        output_xml_path (str): 输出 XML 文件的保存路径。
    """
    ...
    # 提取所有非背景变化区域的精确轮廓信息
    all_change_results = get_change_area_contours(image_array, geo_transform, proj_wkt)

    # 创建 XML 根元素
    root = ET.Element("Targets")
    
    # 添加检测到的目标总数
    detected_number = ET.SubElement(root, "DetectedNumber")
    detected_number.text = str(len(all_change_results))
    
    # 添加每个目标的信息
    for idx, target_info in enumerate(all_change_results, start=1):
        target_elem = ET.SubElement(root, "Target")
        
        # 目标编号
        target_id = ET.SubElement(target_elem, "TargetID")
        target_id.text = str(idx)
        
        # 变化情况
        target_change = ET.SubElement(target_elem, "TargetChange")
        target_change.text = target_info["变化代码"]
        
        # 坐标系
        coordinate = ET.SubElement(target_elem, "Coordinate")
        coordinate.text = "geodegree"  # 使用地理坐标系
        
        # 轮廓点信息
        points_elem = ET.SubElement(target_elem, "Points")
        
        # 添加所有轮廓点，确保首尾闭合
        geo_points = target_info["变化区域经纬度坐标"]
        for point in geo_points:
            point_elem = ET.SubElement(points_elem, "Point")
            point_elem.text = f"{point[0]:.6f}, {point[1]:.6f}"
        
        # 确保轮廓闭合：添加第一个点作为最后一个点
        if len(geo_points) > 0:
            first_point = geo_points[0]
            point_elem = ET.SubElement(points_elem, "Point")
            point_elem.text = f"{first_point[0]:.6f}, {first_point[1]:.6f}"
    ...
    final_xml_path = output_xml_path
    # 确保输出目录存在
...
def tif_xml(input_folder, output_folder):
    """
    批量将TIF文件转换为XML格式。
    
    参数:
        input_folder (str): 输入文件夹路径，包含TIF文件
        output_folder (str): 输出文件夹路径，用于保存XML文件
    """
    ...
    for filename in tif_files:
        ...
        # 创建对应的输出XML文件名（保持原始文件名，只修改扩展名）
        xml_filename = os.path.splitext(filename)[0] + ".xml"
        output_xml_file = os.path.join(output_folder, xml_filename)
        
        
        # 运行转换
        success = convert_tif_to_xml(input_tif_file, output_xml_file)    



def tif_xml_single(input_tif_path, output_xml_path):
    """
    将单个TIF文件转换为XML格式，并保存到指定的输出路径。
    这是您要求的新增函数。

    参数:
        input_tif_path (str):  第一个参数，输入的TIF文件完整路径。
        output_xml_path (str): 第二个参数，希望保存的XML文件的完整路径。
    ...
    # a. 准备工作：确保输出目录存在
    output_folder = os.path.dirname(output_xml_path)
    if output_folder:
        os.makedirs(output_folder, exist_ok=True)
    
    # b. 调用您已有的核心转换函数
    success = convert_tif_to_xml(input_tif_path, output_xml_path)
    
    if success:
        print(f"saved successfully: {output_xml_path}")
    else:
        print(f"failed to save {input_tif_path}")
...
if __name__ == "__main__":
    ...
    if type1 == 0:
        input_tif_file = "Khartoum_airport_01_clf.tif"
        output_xml_file = "change_results.xml"
    
        # 运行转换
        convert_tif_to_xml(input_tif_file, output_xml_file)
    
        # if os.path.exists(output_xml_file):
        #     print("\n--- 生成的 XML 文件内容预览 ---")
        #     with open(output_xml_file, 'r', encoding='utf-8') as f:
        #         print(f.read())
        #     print("--------------------------------")
    else:
        # 设置输入文件夹和输出文件夹路径
        input_folder = r"C:\Users\elenstian\Desktop\模型训练结果展示 - 副本\战损检测运行结果tif"  # 替换为你的TIFF文件夹路径
        output_folder = r"C:\Users\elenstian\Desktop\模型训练结果展示 - 副本\战损检测运行结果xml"  # 替换为你的XML输出文件夹路径
        ...
        print(f"找到 {len(tif_files)} 个TIFF文件需要转换...")
        
        # 使用进度条显示转换进度
        for filename in tif_files:
            input_tif_file = os.path.join(input_folder, filename)
            
            # 极速对应的输出XML文件名（保持原始文件名，只修改扩展名）
            xml_filename = os.path.splitext(filename)[0] + ".xml"
            output_xml_file = os.path.join(output_folder, xml_filename)
            
            print(f"\n正在处理 {filename} → {xml_filename}")
            
            # 运行转换
            success = convert_tif_to_xml(input_tif_file, output_xml_file)
            
            if success and os.path.exists(output_xml_file):
                # 预览XML文件头部内容
                print("\n--- XML文件头部内容预览 ---")
                with open(output_xml_file, 'r', encoding='utf-8') as f:
                    # 只读取前10行作为预览
