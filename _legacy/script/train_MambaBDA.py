...
class Trainer(object):
    ...
    def training(self):
        ...
        for _ in tqdm(range(elem_num), desc="Training", dynamic_ncols=True, leave=True):
            ...
            clf_class_counts = [129755132,15916349,20106923,9504770]
            clf_weights = 1.0 / np.sqrt(np.array(clf_class_counts)) 
            clf_weights = torch.FloatTensor(clf_weights / clf_weights.sum()).cuda()
            ...
            ce_loss_clf = F.cross_entropy(output_clf, labels_clf,weight=clf_weights , ignore_index=255)
    ...
    def validation(self):
        ...
        with torch.no_grad():
            for itera, data in enumerate(tqdm(val_data_loader, ncols=50, dynamic_ncols=True,mininterval=1.0)):
                pre_change_imgs, post_change_imgs, labels_loc, labels_clf, _ = data
